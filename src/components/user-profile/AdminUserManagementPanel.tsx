...
import type { TenantUser } from "@/types/user-profile"
...
export function AdminUserManagementPanel({ tenantId }: AdminUserManagementPanelProps) {
  const { isAdmin } = useCurrentUser()
  const { data: tenantUsersData = [], isLoading } = useTenantUsers(tenantId, isAdmin)
  ...
  const team = safeArray(tenantUsersData) as TenantUser[]
    ...
    <Card className="card-elevated rounded-[1.25rem] border border-border bg-card shadow-card transition-all duration-300 hover:shadow-lg">
      ...
      <CardContent>
        ...
        {filteredTeam.length === 0 ? (
          <div
            className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border bg-muted/20 px-6 py-12 text-center"
            role="status"
          >
